...
function authenticate_creditor(){
    if(isset($_SESSION['card_id'], $_SESSION['pin']))
       {
         include 'connection.php';
          $pdo = new PDO($dsn, $user, $pass, $opt);
          try {
              $stmt = $pdo->prepare('SELECT * FROM users WHERE phone = :user');
          } catch(PDOException $ex) {
              echo "An Error occured!"; 
              print_r($ex->getMessage());
          }
    ...
    if(isset($_SESSION['administrator'], $_SESSION['loged_primitives']))
      {
         include 'connection.php';
          $pdo = new PDO($dsn, $user, $pass, $opt);
          try {
              $stmt = $pdo->prepare('SELECT * FROM login_admin WHERE user = :user');
          } catch(PDOException $ex) {
              echo "An Error occured!"; 
              print_r($ex->getMessage());
          }
   ...
   function clean($data)
    ...
    function see_status($data){
     ...
     function see_status2($data){
      if(isset($_REQUEST['choice'])){
         if($data['choice']=="success")
        { echo '<div  style="padding:10px;color:#fff;background-color:#4caf50;text-transform:capitalize;text-align:center">'.$data['value'].'</div><br/>'; }
        else { echo '<div  style="padding:10px;color:#fff;background-color:#f44336;text-transform:capitalize;text-align:center">'.$data['value'].'</div><br/>';   }
      }
    }
