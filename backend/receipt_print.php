...
<script src="//code.jquery.com/jquery-1.11.1.min.js"></script>
 ...
 <?php 
    try {
          $stmt = $pdo->prepare('SELECT * FROM `member` WHERE `id`="'.base64_decode($_REQUEST['8d1d4357e1e1c6b3e4ea6df4ff01fede']).'" ');
          ...
          echo'</br>
                ...
                <tr>
                  ...
                  <td style="padding:10px;text-align:right">
                     <b>Member Id:</b> '.$value['member_id'].'</br>
                     <b>Package:</b> '.$value['membership_type'].'</br>
                     <b>Name:</b> '.$value['member_name'].'</br>
                     <b>Invoice Number:</b> '.$i.'</br>
                     <b>Phone No: </b> '.$value['phone_number'].'</br>
                     <b>Address: </b> '.$value['address'].' </br>
                     <b>Age: </b> '.$value['age'].'</br> 
                     <b>Validity: </b> '.$value['validity_from'].' <b>To</b> '.$value['validity_to'].'</br> 
                     <b>Date : </b> '.$value['date'].'
                  </td>
                </tr>
              </table>';
      ...
      <table style="width:100%">
        ...
        <?php
          try {
              $stmt = $pdo->prepare('SELECT * FROM `payment` WHERE `member_id`="'.$value['member_id'].'" ');
          } catch(PDOException $ex) {
              echo "An Error occured!"; 
              print_r($ex->getMessage());
          }
          ...
          foreach ($user as $key => $value1) {
            echo '<tr>
                    ...
                    <td>Installment - '.$i.'</td>                    
                    <td>'.$value1['date'].'</td>
                    <td>'.$value1['paid_by'].'</td>
                    <td>Rs.'.$value1['pay_amount'].'.00</td>
                  </tr>';
        ...
        <tr style="background-color: #eee;">
          ...
          <td style="font-size: 16px;font-weight: bold;border:0;padding:5px;"> Rs.  <?php echo $value['due_amount']; ?>.00</td>
        </tr>
